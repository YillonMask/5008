        ...
        for(j=0; j < COLUMNS; j++){
            if(i==j){
                g[i*COLUMNS+j] = 0; // Why do we have this condition? 
                                    // cause node can't connect itself
            }else{
...
int nodeInDegree(int* g, int node){
    // TODO
    // for in-degree, we check the sum of the column of the node
    int res = 0;
    for(int i = 0; i < ROWS; i++){
        res += g[i * COLUMNS + node];
    }
...
int nodeOutDegree(int* g, int node){
    // TODO
    // for out-degree, we check the sum of the row of the node
    int res = 0;
